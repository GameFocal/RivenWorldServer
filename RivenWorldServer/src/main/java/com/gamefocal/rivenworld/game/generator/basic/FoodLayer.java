...
import com.gamefocal.rivenworld.game.entites.resources.fruit.BerryBush;
    ...
    public void generateLayer(World world) {
        ...
        for (int x = 0; x < grid.getWidth(); x++) {
            for (int y = 0; y < grid.getHeight(); y++) {
                ...
                if (cell > .60 && height >= 4500 && RandomUtil.getRandomChance(.001)) {
                    Location worldLoc = world.getRawHeightmap().getWorldLocationFromXY(x, y);

                    WorldMetaData metaData = world.getRawHeightmap().getMetaDataFromXY(worldLoc);
