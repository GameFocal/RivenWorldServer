...
import com.gamefocal.rivenworld.game.entites.resources.ground.BirdNestEntity;
...
public class BirdNestLayer implements WorldLayerGenerator {
    @Override
    public void generateLayer(World world) {
        ...
        for (int x = 0; x < grid.getWidth(); x++) {
            for (int y = 0; y < grid.getHeight(); y++) {
                ...
                if (cell > .55 && height >= 10000 && RandomUtil.getRandomChance(.01)) {
                    ...
                    if (metaData.getForest() == 1) {
                        ...
                        resourceNode.spawnEntity = new BirdNestEntity();
